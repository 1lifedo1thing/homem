...
- In multi-pane mode, tapping or scrolling content hides only the pane title bars. The workspace navigation bar stays visible. Tap a centered top-edge handle to reveal them again. The Add pane menu offers **Keep pane bars visible**; VoiceOver also keeps them visible. Header controls and dragging do not hide the bars, and Reduce Motion disables the transition.
