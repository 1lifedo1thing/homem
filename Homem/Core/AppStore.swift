...
@MainActor @Observable final class AppStore {
    ...
    func connect(address: String, username: String, password: String, accessToken: String) async throws {
        ...
        profile = user; accountProfile = .null; bots = []; workspace = .null; api = client
        await reload()
    }
    func reload() async {
    ...
    private func loadOfficialIdentity(_ client: APIClient) async {
        ...
        if let result {
            workspace = OfficialIdentity.teams(result).first { $0["team_id"].string == client.officialSession?.teamID } ?? .null
        }
    ...
    func connectOfficial(client: APIClient, teamID: String, workspace: JSONValue = .null) async throws {
        ...
        profile = user; accountProfile = .null; bots = botValue.items.map(Record.init); selectedBotID = bots.first?.id ?? ""; api = client
    ...
    func signOut() {
        ...
        api?.session.invalidateAndCancel(); api = nil; bots = []; profile = .null; accountProfile = .null; workspace = .null; error = nil
    }
}
