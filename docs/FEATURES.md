...
| Chat sessions | Message-first creation with attachments, named run locations, and enabled Memoh/Codex/Claude Code/ACP agent selection; runtime icons and labels in chat rows; list/page/rename/delete with live running indicators, Markdown/code, history, reasoning/model choices | Conversation settings provides rename, summarization, context usage, and capability-gated goal controls; arbitrary ACP commands are not exposed |
...
| Message operations | Retry, edit, fork when server marks the turn forkable; queued follow-ups and steering with edit, remove, reorder, and promote controls | Sending while a response is active queues text. Memoh’s queue is text-only; attachments stay in the composer. Accepted items reload from the server on reopening; uncertain retries reuse their invocation ID. |
...
| Desktop | Native RFB through the official runtime gateway; native WebRTC for third-party servers; pointer drag/click, scroll, right-click, text and control keys; native pinch zoom (1–6×), two-finger canvas pan, one-finger view-only pan, and Fit to screen; key controls move into unused side space on wide panes; system Picture in Picture for live, view-only playback | Official Catalyst rendering/input/reconnect verified; third-party WebRTC requires working ICE/network reachability; no browser-engine embedding |
...
Use the PiP button beside the desktop controls to open the live desktop in a system video window. The original pane then offers two buttons: **Close pane, keep PiP** or **Show in both**. The latter keeps an interactive inline desktop alongside the view-only floating player, using the same remote connection and incoming frames. Pausing PiP does not freeze the inline desktop. The player supports pause/resume and returning to the desktop. PiP keeps the connection alive while the app is backgrounded or the original pane is left; closing it releases the connection when no viewer remains. Switching accounts closes PiP and clears its frame. It uses the same sample-buffer surface for official RFB and third-party WebRTC, with no camera, microphone, or artificial audio stream.

Validation: nine focused simulator tests cover RGB/I420 frame conversion, rotated WebRTC frames, live sample timestamps, pause/skip callbacks, zoom/pan/layout behavior, and duplicate-surface ownership across fullscreen transitions. The iOS 27.1 Duo simulator reports system PiP unsupported, so the floating window and background playback still need a supported-device check. The button is hidden where the system reports no PiP support.
