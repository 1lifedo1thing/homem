...
| Desktop | Native RFB through the official runtime gateway; native WebRTC for third-party servers; pointer drag/click, scroll, right-click, text and control keys; native pinch zoom (1–6×), two-finger canvas pan, one-finger view-only pan, and Fit to screen | Official Catalyst rendering/input/reconnect verified; third-party WebRTC requires working ICE/network reachability; no browser-engine embedding |
