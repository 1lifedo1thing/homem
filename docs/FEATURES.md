...
Use the PiP button beside the desktop controls to open the live desktop in a system video window. The player supports pause/resume and returning to the desktop. PiP keeps the connection alive while the app is backgrounded or the original pane is left; closing it releases the connection when no viewer remains. Switching accounts closes PiP and clears its frame. It uses the same sample-buffer surface for official RFB and third-party WebRTC, with no camera, microphone, or artificial audio stream.

Validation: eight focused simulator tests cover RGB/I420 frame conversion, rotated WebRTC frames, live sample timestamps, pause/skip callbacks, and zoom/pan/layout behavior. The iOS 27.1 Duo simulator reports system PiP unsupported, so the floating window and background playback still need a supported-device check. The button is hidden where the system reports no PiP support.
